...
func makeAsyncContentEvaluator(ctx context.Context, content []*Content) (*asyncContentEvaluator, diagnostics.Diag) {
	...
	for _, c := range content {
		diag := assignAsyncContent(ctx, c, rootNode, rootNode, namedMap, invokeMap)
...
func assignAsyncContent(
	...
	case c.Plugin != nil:
		...
		ac := makeAsyncContent(ctx, rootNode, parent, c, dependsOn)
		namedMap[c.Plugin.BlockName] = ac
		invokeMap[c.Plugin.Provider.InvocationOrder] = append(invokeMap[c.Plugin.Provider.InvocationOrder], ac)
	case c.Section != nil:
		tmp := plugin.NewSection(0)
		parent.Add(tmp, nil)
		for _, child := range c.Section.children {
			diag := assignAsyncContent(ctx, child, rootNode, tmp, namedMap, invokeMap)
			if diags.Extend(diag) {
				return diag
			}
		}
